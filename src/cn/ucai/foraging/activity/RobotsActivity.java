...
public class RobotsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(cn.ucai.foraging.R.layout.fragment_robots);
		inputMethodManager = (InputMethodManager) getSystemService(INPUT_METHOD_SERVICE);
		mListView = (ListView) findViewById(cn.ucai.foraging.R.id.list);
		swipeRefreshLayout = (SwipeRefreshLayout) findViewById(cn.ucai.foraging.R.id.swipe_layout);
		swipeRefreshLayout.setColorSchemeResources(android.R.color.holo_blue_bright, android.R.color.holo_green_light,
				android.R.color.holo_orange_light, android.R.color.holo_red_light);
		progressBar = findViewById(cn.ucai.foraging.R.id.progress_bar);
	...
	class RobotAdapter extends ArrayAdapter<RobotUser> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = inflater.inflate(cn.ucai.foraging.R.layout.row_robots, null);
			}
			((TextView) convertView.findViewById(cn.ucai.foraging.R.id.name)).setText(getItem(position).getNick());
			return convertView;
		}
