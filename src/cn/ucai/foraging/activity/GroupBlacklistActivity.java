...
public class GroupBlacklistActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(cn.ucai.foraging.R.layout.activity_group_blacklist);

		progressBar = (ProgressBar) findViewById(cn.ucai.foraging.R.id.progressBar);
		listView = (ListView) findViewById(cn.ucai.foraging.R.id.list);

		groupId = getIntent().getStringExtra("groupId");
		// 注册上下文菜单
		registerForContextMenu(listView);
		final String st1 = getResources().getString(cn.ucai.foraging.R.string.get_failed_please_check);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		getMenuInflater().inflate(cn.ucai.foraging.R.menu.remove_from_blacklist, menu);
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == cn.ucai.foraging.R.id.remove) {
	...
	void removeOutBlacklist(final String tobeRemoveUser) {
		final String st2 = getResources().getString(cn.ucai.foraging.R.string.Removed_from_the_failure);
		try {
			// 移出黑民单
	...
	private class BlacklistAdapter extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = View.inflate(getContext(), cn.ucai.foraging.R.layout.row_contact, null);
			}

			TextView name = (TextView) convertView.findViewById(cn.ucai.foraging.R.id.name);
